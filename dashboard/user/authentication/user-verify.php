...
if(isset($_GET['id']) && isset($_GET['code']))
{
 ...
 $stmt = $user->runQuery("SELECT userId,status FROM student WHERE userId=:uID AND tokencode=:code LIMIT 1");
 ...
 if($stmt->rowCount() > 0)
 {
  ...
  {
   $stmt = $user->runQuery("UPDATE student SET status=:status WHERE userId=:uID");
